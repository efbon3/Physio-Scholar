...
- **Descriptive** — a text area for typing the answer. The student may type, edit freely, or leave the textarea blank if they answered mentally or wrote on paper — Submit accepts an empty answer and proceeds to reveal so the learner can self-rate against the model answer regardless of how they engaged. The engagement-method prompt after self-rating captures the difference (wrote on screen / wrote on paper / mental). A separately subordinate **"I don't know"** button is the *no-engagement* path: tapping it skips straight to the model-answer reveal and auto-rates the card as Don't know after the 5-second reading delay (no self-rating step). There is no character limit.
...
A primary **"Submit answer"** button is the action that commits the student's input and advances to the reveal screen. The student's input (or "I don't know" selection, where applicable) is locked at the moment of submit; the reveal screen is purely read-only from there. For descriptive, an empty text area at submit is permitted and indicates the student answered off-screen (mentally or on paper) — they still self-rate against the model answer, and the engagement-method prompt records the engagement style. The explicit "I don't know" button is a separate path for *no engagement at all* — it auto-rates the card as Don't know after the 5-second reading delay, no self-rating.
