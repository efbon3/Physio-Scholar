...
import { parseMechanism, splitLayers, type Mechanism } from "./loader";
...
export async function readMechanismById(id: string): Promise<Mechanism | null> {
  ...
  // Routes through `readAllMechanisms` so the chapter-merge logic
 ...
 * matching the UI's design intent that one mechanism carries all
 * three test formats.
 *
 * Mergeability is keyed on `frontmatter.title` (which the chapter
 * parser sets to the verbatim `chapter:` field). Canonical mechanism
...
function mergeOneGroup(group: readonly Mechanism[]): Mechanism {
  ...
  // under the primary's mechanism_id namespace.
  ...
  // Canonical mechanism files don't merge — they're filtered upstream
